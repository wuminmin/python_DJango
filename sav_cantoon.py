...
from canteen.models import 订餐主界面表, 订餐食堂模版表, 订餐部门表,订餐结果表
...
def 导入食堂模版(文件名):
    ...
    for index_main, row in df_main.iterrows():
        ...
        食堂模版表_one = 订餐食堂模版表.objects(主菜单name=主菜单name, 子菜单page_name=子菜单page_name, 子菜单page_desc=子菜单page_desc).first()
        if 食堂模版表_one == None:
            订餐食堂模版表(主菜单name=主菜单name, 主菜单id=主菜单id,
...
def 导入主页数据(文件名):
    ...
    for 手机号 in 手机号_list:
        ...
        for index, row in df_手机号.iterrows():
            ...
            主界面表_one = 订餐主界面表.objects(手机号=str(手机号)).first()
            if 主界面表_one == None:
                订餐主界面表(手机号=str(手机号), 描述=str(描述), 创建时间=str(创建时间), 主页标题=str(主页标题), 主页描述=str(主页描述), 验证码标题=str(验证码标题)
...
def 保存订餐部门列表():
    订餐主界面表_all = 订餐主界面表.objects
    二级部门list = []
    for 订餐主界面表_one in 订餐主界面表_all:
        if 订餐主界面表_one.二级部门 in 二级部门list:
            pass
        else:
            二级部门list.append(订餐主界面表_one.二级部门)
    for 二级部门one in 二级部门list:
        订餐主界面表_二级部门 = 订餐主界面表.objects(二级部门=二级部门one)
        三级部门list = []
        for 订餐主界面表_二级部门one in 订餐主界面表_二级部门:
            if 订餐主界面表_二级部门one.三级部门 in 三级部门list:
                pass
            else:
                三级部门list.append(订餐主界面表_二级部门one.三级部门)
        订餐部门表_firtst = 订餐部门表.objects(二级部门=二级部门one).first()
        if 订餐部门表_firtst == None:
            订餐部门表(
                二级部门=二级部门one,
                三级部门列表=三级部门list
            ).save()
...
def 导出订餐人员():
    ...
    for 订餐主界面表_one in 订餐主界面表_objs:
        col1.append(订餐主界面表_one.手机号)
        col2.append(订餐主界面表_one.描述)
        col3.append(订餐主界面表_one.主页标题)
        col4.append(订餐主界面表_one.主页描述)
        col5.append(订餐主界面表_one.验证码标题)
        col6.append(订餐主界面表_one.验证码描述)
        col7.append(订餐主界面表_one.二级部门)
        col8.append(订餐主界面表_one.三级部门)
        col9.append(订餐主界面表_one.四级部门)
        col10.append(订餐主界面表_one.姓名)
...
def 根据三级部门删除主界面表(三级部门):
    订餐主界面表objs = 订餐主界面表.objects(三级部门=三级部门)
    for one in 订餐主界面表objs:
        print(one.三级部门)
        one.delete()
